...
def _top_k_accuracy(size, k, seed):
    ...
    harmonic_tax_model = TaxonomicLossModel.from_pretrained(f"./models/{size}-tax-harmonic-{seed}",
                                                            num_labels=len(glosses))
    harmonic_tax_model.use_morphology_tree(morphology, 5)

    flat_model = AutoModelForTokenClassification.from_pretrained(f"./models/{size}-flat-{seed}",
                                                                 num_labels=len(glosses))
