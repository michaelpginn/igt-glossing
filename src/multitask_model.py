...
class MultitaskModel(RobertaForTokenClassification):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            for head_index, logits in enumerate(all_classifier_logits):
                level_labels = labels.select(1, head_index).reshape(-1)
                level_loss = loss_fct(logits.view(-1, self.classifier_head_sizes[head_index]), level_labels)
                print(f"Level {head_index}:", level_loss)
